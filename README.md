...
> [!WARNING]
>
> Every chart with a chart version of `0.x.y` is in pre-1.0 development. Breaking changes to values, rendered resources,
> defaults or application behavior may ship in minor chart releases (`0.x.y` → `0.(x+1).0`), with migration steps in the
> chart's Upgrading section. Pin the chart version and review its release notes and upgrade guidance before updating.
> Each chart reaches stable `1.0.0` independently, after maintainer validation and testing; an application version of
> `1.0.0` or higher does not make its chart stable. See the [chart versioning policy][chart_versioning].
...
[chart_versioning]: docs/CONTRIBUTING.md#versioning
